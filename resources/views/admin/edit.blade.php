...
<div class="desk-form-page">
    <div class="desk-form-header">
        ...
        <h1 class="desk-form-title">edit entry</h1>
    </div>
    ...
    <div class="desk-form-card">
        <form action="{{ route('admin.update', $blogPost) }}" method="POST">
            ...
            <div class="desk-form-group">
                ...
                <input type="text" name="title" id="title" class="desk-form-control" value="{{ old('title', $blogPost->title) }}" required>
                @error('title') <span class="desk-form-error">{{ $message }}</span> @enderror
            </div>

            <div class="desk-form-group">
                <label for="category">Category</label>
                <input type="text" name="category" id="category" class="desk-form-control" value="{{ old('category', $blogPost->category) }}" placeholder="e.g. musings, poetry, diary">
            </div>

            <div class="desk-form-group">
                <label for="excerpt">Excerpt <span style="font-weight: 300; opacity: 0.4;">(a short preview)</span></label>
                <textarea name="excerpt" id="excerpt" class="desk-form-control sm" rows="3">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
            </div>

            <div class="desk-form-group">
                <label for="featured_image">Featured image URL <span style="font-weight: 300; opacity: 0.4;">(optional)</span></label>
                <input type="url" name="featured_image" id="featured_image" class="desk-form-control" value="{{ old('featured_image', $blogPost->featured_image) }}" placeholder="https://images.unsplash.com/...">
            </div>

            <div class="desk-form-group">
                <label for="content">Content</label>
                <textarea name="content" id="content" class="desk-form-control" required>{{ old('content', $blogPost->content) }}</textarea>
            </div>
