...
@section('mgr-content')
    <div class="mgr-card" style="max-width:720px;">
        <form action="{{ $post ? route('manager.post-update', $post) : route('manager.post-store') }}" method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                @error('title')
                    <div style="color:var(--mgr-danger);font-size:0.75rem;margin-top:4px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="display:flex;gap:10px;margin-top:24px;">
                ...
                <a href="{{ route('manager.posts') }}" class="btn">Cancel</a>
            </div>
        </form>
    </div>
@endsection
